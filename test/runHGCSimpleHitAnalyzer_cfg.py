...
process = cms.Process("HGCSimpleHitAnalyzer")
...
import os,sys
if(len(sys.argv)<3):
    print '\ncmsRun runHGCSimHitsAnalyzer_cfg.py tag first_file step\n'
...
process.TFileService = cms.Service("TFileService", fileName = cms.string('SimpleHitAnalysis.root'))
process.analysis = cms.EDAnalyzer("HGCSimpleHitAnalyzer",
                                  hitCollections           = cms.vstring('HGCHitsEE',               'HGCHitsHEfront', 'HcalHits'),
                                  geometrySource           = cms.vstring('HGCalEESensitive',        'HGCalHESiliconSensitive', 'HCal')
                                  )
